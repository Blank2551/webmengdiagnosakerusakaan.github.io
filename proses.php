...
//mencari id kerusakan berdasarkan gejala yang di inputkan
...
//jika di temukan akan menampilkan info dan solusi dari kerusakan yang di alami
    if($status==true){
        while ($d=mysqli_fetch_array($data)) {
            $id=$d['id'];
        }
        $cari_kerusakaan="SELECT * FROM tb_kerusakaan WHERE id=$id";
        $db=mysqli_query($kon,$cari_kerusakaan);
        while ($d=mysqli_fetch_array($db)){
            $kerusakaan=$d['kerusakaan'];
            $info=$d['info'];
            $solusi=$d['solusi'];
            include 'hasil.php';
        }
